        ...
        foreach ($ids as $id) {
            ...
            foreach ($skustones as $skustone) {
                $stones[] = array('pieces' => $skustone->pieces, 'setting' => $skustone->idsetting0->name, 'color' => $skustone->idstone0->color,
                    ...
                    'weight' => $skustone->idstone0->weight,  'country' => $skustone->idstone0->idstonem0->scountry, 'cost' => $skustone->idstone0->curcost, 'month' => $skustone->idstone0->month);
            }
       ...
       //echo "<pre>";print_r($totalsku);die;
            ...
            if(stripos($totalsku[($i - 8) / 16][6] , 'Gold')!== false){
                ...
                $metalname = 'Gold';
                 ...
                 $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    $metalname = 'Silver';
            }elseif(stripos($totalsku[($i - 8) / 16][6] , 'Brass')!== false){
                 $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('C'.($i + 0), 'Brass')
                    ->setCellValue('C' . ($i + 3),'Brass')
                    ->setCellValue('T' . ($i + 1), 'Brass '.$totalsku[(($i - 8) / 16)][1]['type']);
                    $metalname = 'Brass';
            }
        ...
        $objPHPExcel->getActiveSheet()->getStyle('D'.($i + 3).':D'.($i + 15))->getNumberFormat()->setFormatCode('#,##0.#0');
         ...
         $objPHPExcel->getActiveSheet()->getStyle('S'.($i + 2).':S'.($i + 15))->getNumberFormat()->setFormatCode('#,##0.#0');
            ...
            $objPHPExcel->getActiveSheet()->getStyle('U'.($i + 15))->getNumberFormat()->setFormatCode('"$"#,##0.#"0"');
